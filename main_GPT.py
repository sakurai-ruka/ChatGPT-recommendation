...
while input != "quit()":
    ...
    if len(keyword2)>0:
        ...
        html = response.text
        soup = BeautifulSoup(html, 'html.parser')
        items = soup.find('li', class_='SearchResults_SearchResults__page__2DR0s')
        try:
            name = items.find_all('a', class_='SearchResult_SearchResult__detailsContainerLink__0hDP4')
        #商品が存在しなかった場合の例外処理
